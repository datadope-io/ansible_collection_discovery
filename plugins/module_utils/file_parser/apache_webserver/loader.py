...
from ansible_collections.datadope.discovery.plugins.module_utils.file_parser.apache_webserver.errors import *
from ansible_collections.datadope.discovery.plugins.module_utils.file_parser.apache_webserver.lexer import make_lexer
from ansible_collections.datadope.discovery.plugins.module_utils.file_parser.apache_webserver.parser import make_parser
...
def make_loader(configfile='', **kwargs):
    options = {
        'file': configfile,
        ...
        'configpath': [os.path.dirname(configfile), os.path.dirname(os.path.dirname(configfile))],
...
class ApacheConfigLoader(object):
    ...
    def __init__(self, parser, debug=True, **options):
        self._parser = parser
        self._debug = debug
        ...
        self._reader._environ.update(self._options['envvars'])
        self._path_prefix = self._options['pathprefix']
        self._stack = []
        self._includes = set()
        self._ast_cache = {}
    ...
    def _interpolate_vars(self, option, value):
        ...
        if self._options.get('interpolatevars', False):
            def lookup(match):
                ...
                if self._options.get('strictvars', False):
    ...
    def g_jkworkersfile(self, ast):
        filepath = self._unquote_tag(ast[0])
        filepaths = []
        filepaths = self._search_path(filepath)

        contents = {}

        for filepath in filepaths:
            items = self.load(filepath, initialize=False)
            self._merge_contents(contents, items)

        worker_list = [s.strip() for s in contents['worker.list'].split(",")]
        loadbalancers = {}
        upstreams = {}
        for key, value in contents.items():
            try:
                directive, entity, subkey = key.split(".")
            except:
                continue
            # We just parse worker.* entries
            if directive == "worker":
                # If entity in worker.list is loadbalancerser else is an
                # upstream
                if entity in worker_list:
                    if entity not in loadbalancers:
                        loadbalancers[entity] = {}
                    loadbalancers[entity]['name'] = entity
                    loadbalancers[entity][subkey] = value
                else:
                    if entity not in upstreams:
                        upstreams[entity] = {}
                    upstreams[entity]['name'] = entity
                    upstreams[entity][subkey] = value

        return {"JKWorkers": {"JKLoadbalancers": loadbalancers, "JKBalanceWorkers": upstreams}}

    def _search_path(self, filepath):
        ...
        configpath = [self._path_prefix + x if not x.startswith(self._path_prefix) else x
                      for x in configpath]
            ...
            if self._reader.isdir(filepath):
                if options.get('includedirectories'):
                    ...
                    for include_file in sorted(self._reader.listdir(filepath)):
                        filepaths.append(os.path.join(filepath, include_file))

            elif options.get('includeglob'):

                for include_file in sorted(glob.glob(filepath)):
                    filepaths.append(include_file)
        ...
        # Since wildcards may not match anything in empty directories, we avoid raising
        # an exception when no files are found if a wildcard is used.
        if len(filepaths) == 0 and '*' not in filename:
    ...
    def _dumpdict(self, obj, indent=0, continue_tag=False):
        if not isinstance(obj, dict):
            raise ApacheConfigError('Unknown object type "%r" to dump'
                                    % obj)
