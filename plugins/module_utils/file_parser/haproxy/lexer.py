...
class BaseHAProxydConfigLexer(object):
...
#            errorlog=log if self._debug else None
    ...
    def t_CONTEXT(self, t):
        r'[^\n\r\t=\#]?((defaults|frontend|backend|global|listen)[\ ]*([\w:\ \-\.])*)'
        #r'[^\n\r\t=\#]?((default|frontend|backend|global|listen).*)'
        #r'[^ \n\r\t=\#]+([ \t=]+(default|frontend|backend|global|listen)\ .*)'
        #r'(?=([\w\.\/=\ ]*[ \t=]+)*\{)([\w\.\/=\ ]*)*'
...
def make_lexer(**options):
    lexer_class = BaseHAProxydConfigLexer

    lexer_class = type(str('HAProxyConfigLexer'),
