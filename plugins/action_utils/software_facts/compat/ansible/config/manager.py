...
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.config.data import \
    ConfigData
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.errors.__init__ import \
    AnsibleOptionsError, AnsibleError
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.module_utils.common._collections_compat import \
    Mapping, Sequence
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.module_utils.common.text.converters import \
    to_text, to_bytes, to_native
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.module_utils.common.yaml import \
    yaml_load
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.module_utils.parsing.convert_bool import \
    boolean
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.parsing.quoting import \
    unquote
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.parsing.yaml.objects import \
    AnsibleVaultEncryptedUnicode
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.utils import py3compat
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.utils.path import \
    cleanup_tmp_file, makedirs_safe, unfrackpath
...
class ConfigManager(object):
    ...
    def _loop_entries(self, container, entry_list):
        ...
        for entry in entry_list:
            ...
            try:
                ...
                self.WARNINGS.add(
                    u'value for config entry {0} contains invalid characters, ignoring...'.format(to_text(name)))
                continue
    ...
    def get_config_value(self, config, cfile=None, plugin_type=None, plugin_name=None, keys=None, variables=None,
        ...
        try:
            value, _drop = self.get_config_value_and_origin(config, cfile=cfile, plugin_type=plugin_type,
    ...
    def get_config_value_and_origin(self, config, cfile=None, plugin_type=None, plugin_name=None, keys=None,
        ...
        if config in defs:
            ...
            if direct and config in direct:
                                ...
                                for ini_entry in defs[config]['ini']:
                                    ...
                                    if temp_value is not None:
                                        ...
                                        if 'deprecated' in ini_entry:
                                            self.DEPRECATED.append(('[%s]%s' % (ini_entry['section'], ini_entry['key']),
                ...
                if value is None:
                    if defs[config].get('required', False):
                        ...
                        if plugin_type is None and isinstance(value, string_types) and (
            ...
            if value is not None and 'choices' in defs[config] and defs[config]['choices'] is not None:
                ...
                if invalid_choices:
                    raise AnsibleOptionsError('Invalid value "%s" for configuration option "%s", valid values are: %s' %
                                              (value, to_native(_get_entry(plugin_type, plugin_name, config)),
                                               defs[config]['choices']))
            ...
            raise AnsibleError('Requested entry (%s) was not defined in configuration.' % to_native(
    ...
    def update_config_data(self, defs=None, configfile=None):
        ...
        for config in defs:
            if not isinstance(defs[config], dict):
                raise AnsibleOptionsError(
                    "Invalid configuration definition '%s': type is %s" % (to_native(config), type(defs[config])))
