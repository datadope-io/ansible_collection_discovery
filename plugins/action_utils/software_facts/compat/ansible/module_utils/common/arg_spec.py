...
from ansible_collections.datadope.discovery.plugins.action_utils.software_facts.compat.ansible.module_utils.common.parameters import (
    _ADDITIONAL_CHECKS,
    _get_legal_inputs,
        ...
        for key in sorted(self.argument_spec.keys()):
            ...
            if aliases:
                self._valid_parameter_names.update(
                    ["{key} ({aliases})".format(key=key, aliases=", ".join(sorted(aliases)))])
            else:
    ...
    def validate(self, parameters, *args, **kwargs):
        ...
        try:
            result._aliases.update(
                _handle_aliases(self.argument_spec, result._validated_parameters, alias_warnings, alias_deprecations))
        ...
        try:
            result._unsupported_parameters.update(
                _get_unsupported_parameters(self.argument_spec, result._validated_parameters, legal_inputs))
        except TypeError as te:
        ...
        if result._unsupported_parameters:
            ...
            result.errors.append(
                UnsupportedError(
                    "{0}. Supported parameters include: {1}.".format(unsupported_string, supported_string)))
