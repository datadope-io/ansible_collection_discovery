...
def human_to_bytes(number, default_unit=None, isbits=False):
    ...
    try:
        ...
        raise ValueError(
            "human_to_bytes() can't interpret following number: %s (original input string: %s)" % (m.group(1), number))
    ...
    if len(unit) > 1:
        ...
        if unit_class_name in unit.lower():
            ...
            raise ValueError(
                "human_to_bytes() failed to convert %s. Value is not a valid string (%s)" % (number, expect_message))
