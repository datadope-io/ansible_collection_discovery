...
def get_info_by_sysobject(sysobject_ids_file, snmp_sysobjectid):
    ...
    for tpl, brands in sysobject_ids_file.items():
        result.update(_get_info_by_template(tpl, brands, stripped_oid))
        if 'model' not in result.keys() and tpl != list(sysobject_ids_file.keys())[-1]:
            continue
